const DB_NAME = 'VegetableOrderDB', DB_VERSION = 3, STORE_NAME = 'savedData';
let db = null;
...
async function getSavedList() { return new Promise((res, rej) => { const r = db.transaction(STORE_NAME, 'readonly').objectStore(STORE_NAME).getAll(); r.onsuccess = () => res(r.result); r.onerror = () => rej(r.error); }); }
async function saveData(name, data) { return new Promise((res, rej) => { const r = db.transaction(STORE_NAME, 'readwrite').objectStore(STORE_NAME).add({ name, savedAt: new Date().toISOString(), ...data }); r.onsuccess = () => res(r.result); r.onerror = () => rej(r.error); }); }
async function loadData(id) { return new Promise((res, rej) => { const r = db.transaction(STORE_NAME, 'readonly').objectStore(STORE_NAME).get(id); r.onsuccess = () => res(r.result); r.onerror = () => rej(r.error); }); }
async function deleteData(id) { return new Promise((res, rej) => { const r = db.transaction(STORE_NAME, 'readwrite').objectStore(STORE_NAME).delete(id); r.onsuccess = () => res(); r.onerror = () => rej(r.error); }); }

async function renderSavedList() {
  const list = await getSavedList();
  const el = document.getElementById('saved-list');
  if (list.length === 0) { el.innerHTML = '<div class="no-saved">保存データなし</div>'; return; }
  el.innerHTML = list.map(item => {
    const d = new Date(item.savedAt);
    return '<div class="saved-item" onclick="loadFromDB('+item.id+')"><span class="name">'+escapeHtml(item.name)+'</span><span class="meta">'+(d.getMonth()+1)+'/'+d.getDate()+' '+d.getHours()+':'+String(d.getMinutes()).padStart(2,'0')+'</span><span class="del-btn" onclick="event.stopPropagation();deleteFromDB('+item.id+')">✕</span></div>';
  }).join('');
}
...
let loadedFiles = [], rawData = { data: [], stores: [], products: [], dates: [], suppliers: [] }, productInfo = {};
let productTags = {};
let selectedStores = new Set(), selectedSuppliers = new Set(), selectedFiles = new Set();
let allDatesRaw = [], allProducts = [];
let selectedCols = new Set(), selectedRows = new Set();
...
function parseWorkbook(wb, fileName) {
  const data = [], sheets = [], pInfo = {}, prods = new Set();
  wb.SheetNames.forEach(sn => {
    const json = XLSX.utils.sheet_to_json(wb.Sheets[sn], { header: 1, defval: '' });
    if (json.length < 8) return;
    sheets.push(sn);
    const hdr = json[6] || [], storeCols = [];
    for (let c = 6; c < hdr.length; c++) { const code = hdr[c]; if (code !== '' && code !== null) storeCols.push({ col: c, code: String(Math.floor(Number(code)) || code) }); }
    let curProd = null, curCost = null, curPrice = null, curUnit = null;
    for (let r = 7; r < json.length; r++) {
      const row = json[r]; if (!row || row.length === 0) continue;
      const c0 = String(row[0] || '').trim();
      if (c0.length > 0) {
        curProd = extractProductName(c0); prods.add(curProd);
        const pm = String(row[3] || '').match(/(\d+)[^\d]+(\d+)/);
        if (pm) { curCost = parseInt(pm[1]); curPrice = parseInt(pm[2]); } else { curCost = curPrice = null; }
        const um = String(row[4] || '').match(/(\d+)\s*$/);
        curUnit = um ? parseInt(um[1]) : null;
        if (curProd && !pInfo[curProd]) pInfo[curProd] = { cost: curCost, price: curPrice, unit: curUnit };
      }
      const dm = String(row[2] || '').trim().match(/(\d+)\/(\d+)/);
      if (!dm) continue;
      const ds = parseInt(dm[1]) + '/' + parseInt(dm[2]);
      if (curProd) storeCols.forEach(sc => { const q = Number(row[sc.col]); if (q > 0) data.push({ fileName, supplier: sn, product: curProd, date: ds, store: sc.code, quantity: q }); });
    }
  });
  return { data, sheets, productInfo: pInfo, products: Array.from(prods) };
}
...
function updateFileChips() {
  ...
  document.getElementById('files-count').textContent = loadedFiles.length;
...
function updateTable() {
  ...
  document.getElementById('stat-margin').textContent = marginRate;
