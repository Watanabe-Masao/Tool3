...
async function getSavedList(): Promise<SavedData[]> {
  return new Promise<SavedData[]>((resolve, reject) => {
    if (!db) {
    ...
    request.onsuccess = () => resolve(request.result as SavedData[]);
...
function showSaveModal() {
  if (loadedFiles.length === 0) { showToast('データがありません'); return; }
  (document.getElementById('save-name') as HTMLInputElement).value = loadedFiles.map(f => f.name.replace(/\.[^.]+$/, '')).join(', ');
...
function updateTagDatalist() {
  ...
  Object.keys(productTags).forEach(function(p) {
    if (productTags[p].tag1) tag1Set.add(productTags[p].tag1);
    if (productTags[p].tag2) tag2Set.add(productTags[p].tag2);
    if (productTags[p].tag3) tag3Set.add(productTags[p].tag3);
  });
  document.getElementById('tag1-list')!.innerHTML = Array.from(tag1Set).sort().map(function(t: string) { return '<option value="' + escapeHtml(t) + '">'; }).join('');
  document.getElementById('tag2-list')!.innerHTML = Array.from(tag2Set).sort().map(function(t: string) { return '<option value="' + escapeHtml(t) + '">'; }).join('');
  document.getElementById('tag3-list')!.innerHTML = Array.from(tag3Set).sort().map(function(t: string) { return '<option value="' + escapeHtml(t) + '">'; }).join('');
}

function addProduct() {
  var name = document.getElementById('add-product-name').value.trim();
  if (!name) { showToast('❌ 品目名を入力してください'); return; }
  if (allProducts.indexOf(name) >= 0) { showToast('❌ 同名の商品が既に存在します'); return; }
...
function deleteProduct(product) {
  if (!confirm('「' + product + '」を削除しますか？')) return;
...
async function saveToDatabase() {
  ...
  try { await saveData(name, { loadedFiles, productInfo, allProducts, productTags }); closeSaveModal(); showToast('✅ 保存しました'); renderSavedList(); }
...
async function loadFromDB(id) {
  try {
    ...
    loadedFiles = data.loadedFiles || []; productInfo = data.productInfo || {}; allProducts = data.allProducts || []; productTags = data.productTags || {};
...
async function deleteFromDB(id) {
  ...
  try { await deleteData(id); showToast('🗑️ 削除しました'); renderSavedList(); } catch(e) { showToast('❌ 削除に失敗しました'); }
}
...
/** Selected suppliers */
let selectedSuppliers: Set<string> = new Set();

/** Selected file IDs */
let selectedFiles: Set<string | number> = new Set();

/** All dates from loaded data */
let allDatesRaw: string[] = [];

/** All product names */
let allProducts: string[] = [];

/** Selected column indices */
let selectedCols: Set<number> = new Set();

/** Selected row indices */
let selectedRows: Set<number> = new Set();

/** Column drag state */
let isDraggingCol: boolean = false;

/** Row drag state */
let isDraggingRow: boolean = false;

/** Drag start column index */
let dragStartCol: number = -1;

/** Drag start row index */
let dragStartRow: number = -1;
...
const fileInputMini = document.getElementById('file-input-mini') as HTMLInputElement;
const tooltip = document.getElementById('selection-tooltip') as HTMLElement;
...
fileInputMini.addEventListener('change', (e) => { if (e.target.files.length > 0) { handleFiles(e.target.files); e.target.value = ''; } });

function handleFiles(files: FileList) {
  const valid = Array.from(files).filter((f: File) => ['.xls', '.xlsx'].includes(f.name.substring(f.name.lastIndexOf('.')).toLowerCase()));
  if (valid.length === 0) { alert('Excelファイル（.xls, .xlsx）を選択してください'); return; }
  dropZone.style.display = 'none';
  document.getElementById('loading')!.classList.add('show');
  let done = 0;
  valid.forEach((file: File) => {
    if (loadedFiles.some(f => f.name === file.name)) { done++; if (done === valid.length) finishLoading(); return; }
    ...
    reader.onload = (e: ProgressEvent<FileReader>) => {
      ...
      } catch(err) { alert(file.name + ' の読み込みに失敗しました'); }
      done++;
      if (done === valid.length) finishLoading();
    };
...
function parseWorkbook(wb: any, fileName: string): ParsedFileData {
  ...
  const sheets: string[] = [];
  const pInfo: Record<string, ProductInfo> = {};
  const prods = new Set<string>();

  wb.SheetNames.forEach((sn: string) => {
    const json = window.XLSX.utils.sheet_to_json(wb.Sheets[sn], { header: 1, defval: '' }) as any[][];
    if (json.length < 8) return;
    sheets.push(sn);
    const hdr = json[6] || [];
    const storeCols: StoreColumn[] = [];
...
function mergeAllData() {
  ...
  loadedFiles.forEach(f => {
    ...
    if (f.products) f.products.forEach(p => allProds.add(p));
    if (f.productInfo) Object.assign(productInfo, f.productInfo);
  });
  allDatesRaw = (Array.from(allDates).sort((a: string, b: string) => { const ap = a.split('/').map(Number), bp = b.split('/').map(Number); return (ap[0] === 12 ? 0 : 1) - (bp[0] === 12 ? 0 : 1) || ap[1] - bp[1]; }) as string[]);
  allProducts = Array.from(allProds).sort() as string[];
  rawData = { data: allData, stores: Array.from(allStores).sort((a: string, b: string) => Number(a) - Number(b)) as string[], products: [...new Set(allData.map(d => d.product))].sort() as string[], dates: allDatesRaw, suppliers: Array.from(allSuppliers).sort() as string[] };
...
function initSlider() {
  ...
  sf.value = 0; st.value = maxVal;
  sliderFromIdx = 0; sliderToIdx = maxVal;
  updateSliderUI();
  sf.oninput = function(e: Event) {
...
function removeFile(id) { loadedFiles = loadedFiles.filter(f => f.id != id); selectedFiles.delete(id); if (loadedFiles.length === 0) { clearAllFiles(); return; } mergeAllData(); updateFileChips(); initUI(); document.getElementById('file-filter-panel').style.display = loadedFiles.length > 1 ? 'block' : 'none'; }

function clearAllFiles() {
  loadedFiles = []; rawData = { data: [], stores: [], products: [], dates: [], suppliers: [] }; productInfo = {}; productTags = {};
  selectedStores = new Set(); selectedSuppliers = new Set(); selectedFiles = new Set(); allDatesRaw = []; allProducts = [];
...
function extractProductName(raw) {
  if (raw.indexOf('\t') >= 0) { const parts = raw.split('\t'); if (parts.length >= 2) { const np = parts[parts.length - 1].split(/[\u3000\s]+/).filter(p => p.trim()); if (np.length > 0 && /^0\d+$/.test(np[0])) np.shift(); if (np.length > 0) return np.slice(0, 4).join(' '); } }
  const parts = raw.split(/[\u3000\s]+/).filter(p => p.trim());
  if (parts.length > 3) { const idx = parts.findIndex(p => !/^\d+$/.test(p) && p.length > 1); if (idx >= 0) return parts.slice(idx, idx + 4).join(' '); }
...
function toggleDropdown(type) { const dd = document.getElementById(type + '-dropdown'), isOpen = dd.classList.contains('show'); document.querySelectorAll('.dropdown-menu').forEach(d => d.classList.remove('show')); if (!isOpen) dd.classList.add('show'); }
...
function setTag(product, tagNum, value) {
  if (!productTags[product]) productTags[product] = {};
  ...
  reader.onload = function(e: ProgressEvent<FileReader>) {
    try {
      ...
      if (file.name.endsWith('.csv')) {
        ...
        const result = e.target?.result as ArrayBuffer;
        const wb = XLSX.read(new Uint8Array(result), { type: 'array' });
        data = XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]], { header: 1, defval: '' }) as any[][];
      }
      var headerIdx = 0;
      for (var i = 0; i < Math.min(5, data.length); i++) {
        const row = data[i].map(c => String(c).toLowerCase());
        if (row.some(c => c.indexOf('品目') >= 0 || c.indexOf('商品') >= 0 || c.indexOf('product') >= 0)) { headerIdx = i; break; }
      }
      ...
      if (prodCol === -1) { showToast('❌ 品目名列が見つかりません'); return; }
      ...
      for (var i = headerIdx + 1; i < data.length; i++) {
        ...
        if (matchedProd) {
          ...
          if (tag1Col >= 0) {
            ...
            if (newVal) productTags[matchedProd].tag1 = newVal;
          }
          if (tag2Col >= 0) {
            ...
            if (newVal) productTags[matchedProd].tag2 = newVal;
          }
          if (tag3Col >= 0) {
            ...
            if (newVal) productTags[matchedProd].tag3 = newVal;
          }
          count++;
      ...
      if (changes.length > 0) {
        ...
        if (changes.length > 10) msg += '\n... 他' + (changes.length - 10) + '件';
        alert(msg);
      }
      showToast('✅ ' + count + '件取込（' + updatedCount + '件上書き）');
      updateTable();
    } catch (err) { console.error(err); showToast('❌ ファイルの読み込みに失敗しました'); }
  };
...
function updateTagStats() {
  ...
  currentProducts.forEach(function(p) {
    ...
    const info = (productInfo[p] || {}) as ProductInfo;
    ...
    if (t1) {
      if (!hierarchy[t1]) hierarchy[t1] = { _total: { qty: 0, cost: 0, price: 0 }, _children: {} };
      hierarchy[t1]._total.qty += qty; hierarchy[t1]._total.cost += cost; hierarchy[t1]._total.price += price;
      if (t2) {
        if (!hierarchy[t1]._children[t2]) hierarchy[t1]._children[t2] = { _total: { qty: 0, cost: 0, price: 0 }, _children: {} };
        hierarchy[t1]._children[t2]._total.qty += qty; hierarchy[t1]._children[t2]._total.cost += cost; hierarchy[t1]._children[t2]._total.price += price;
        if (t3) {
          if (!hierarchy[t1]._children[t2]._children[t3]) hierarchy[t1]._children[t2]._children[t3] = { qty: 0, cost: 0, price: 0 };
          hierarchy[t1]._children[t2]._children[t3].qty += qty; hierarchy[t1]._children[t2]._children[t3].cost += cost; hierarchy[t1]._children[t2]._children[t3].price += price;
        }
      }
    } else if (t2) {
      if (!noTag1.tag2s[t2]) noTag1.tag2s[t2] = { _total: { qty: 0, cost: 0, price: 0 }, _children: {} };
      noTag1.tag2s[t2]._total.qty += qty; noTag1.tag2s[t2]._total.cost += cost; noTag1.tag2s[t2]._total.price += price;
      if (t3) {
        if (!noTag1.tag2s[t2]._children[t3]) noTag1.tag2s[t2]._children[t3] = { qty: 0, cost: 0, price: 0 };
        noTag1.tag2s[t2]._children[t3].qty += qty; noTag1.tag2s[t2]._children[t3].cost += cost; noTag1.tag2s[t2]._children[t3].price += price;
      }
    } else if (t3) {
      if (!noTag1.noTag2[t3]) noTag1.noTag2[t3] = { qty: 0, cost: 0, price: 0 };
      noTag1.noTag2[t3].qty += qty; noTag1.noTag2[t3].cost += cost; noTag1.noTag2[t3].price += price;
    }
  });
  ...
  if (t1s.length === 0 && !hasOrphans) { tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;opacity:0.5;">タグが設定されていません</td></tr>'; return; }
...
function updateTable() {
  ...
  filtered.forEach(function(i) {
    if (!pivot[i.product]) { pivot[i.product] = { total: 0 }; dates.forEach(function(d) { pivot[i.product][d] = 0; }); }
  ...
  if (sortOrder === 'tag') {
    products.sort(function(a, b) {
      ...
      if (t3a !== t3b) return t3a.localeCompare(t3b, 'ja');
  ...
  if (!showZero) products = products.filter(function(p) { return pivot[p].total > 0; });
  currentProducts = products;
  var totalCost = 0, totalPrice = 0;
  products.forEach(function(p) {
    const info = (productInfo[p] || {}) as ProductInfo, qty = pivot[p].total;
    const unit = info.unit || 1;
    if (info.cost) totalCost += qty * unit * info.cost;
    if (info.price) totalPrice += qty * unit * info.price;
  });
  ...
  if (showTag1) hdr += '<th class="tag tag1">#大分類</th>';
  if (showTag2) hdr += '<th class="tag tag2">#中分類</th>';
  if (showTag3) hdr += '<th class="tag tag3">#小分類</th>';
  if (showCost) hdr += '<th class="info-cost">原価</th>';
  if (showPrice) hdr += '<th class="info-price">売価</th>';
  if (showUnit) hdr += '<th class="info">入数</th>';
  ...
  if (products.length === 0) { tbody.innerHTML = ''; document.getElementById('no-data').style.display = 'block'; updateTagStats(); return; }
  document.getElementById('no-data').style.display = 'none';
  var canEdit = canEditCells();
  var html = '';
  products.forEach(function(p, ri) {
    const row = pivot[p], info = (productInfo[p] || {}) as ProductInfo;
    const pEsc = p.replace(/'/g, "\\'").replace(/"/g, '&quot;');
    html += '<tr data-row="'+ri+'" data-product="'+pEsc+'"><td class="product" data-row="'+ri+'">'+escapeHtml(p)+'</td>';
    if (showTag1) html += '<td class="tag tag1"><input type="text" value="'+escapeHtml(getTag(p, 1))+'" onchange="setTag(\''+pEsc+'\', 1, this.value)" placeholder="大"></td>';
    if (showTag2) html += '<td class="tag tag2"><input type="text" value="'+escapeHtml(getTag(p, 2))+'" onchange="setTag(\''+pEsc+'\', 2, this.value)" placeholder="中"></td>';
    if (showTag3) html += '<td class="tag tag3"><input type="text" value="'+escapeHtml(getTag(p, 3))+'" onchange="setTag(\''+pEsc+'\', 3, this.value)" placeholder="小"></td>';
    if (showCost) html += '<td class="info info-cost">'+(info.cost || '-')+'</td>';
    if (showPrice) html += '<td class="info info-price">'+(info.price || '-')+'</td>';
  ...
  if (showTag1) html += '<td></td>';
  if (showTag2) html += '<td></td>';
  if (showTag3) html += '<td></td>';
  if (showCost) html += '<td></td>';
  if (showPrice) html += '<td></td>';
  if (showUnit) html += '<td></td>';
...
function setupSelection() {
  ...
  document.querySelectorAll('td.value').forEach(function(td) {
    td.onmousedown = function(e: MouseEvent) {
      if ((e.target as HTMLElement).tagName === 'INPUT') return; // 編集中は無視
...
function applySelectionHighlight() {
  ...
  document.querySelectorAll('td.product').forEach(function(td) { const r = parseInt(td.dataset.row); if (!isNaN(r)) td.classList.toggle('selected-row', selectedRows.has(r)); });
  document.querySelectorAll('td.value').forEach(function(td) {
    const c = parseInt(td.dataset.col), r = parseInt(td.dataset.row);
    td.classList.remove('selected-col', 'selected-cell');
    if (selectedCols.size > 0 && selectedRows.size > 0) { if (selectedCols.has(c) && selectedRows.has(r)) td.classList.add('selected-cell'); }
    else if (selectedCols.size > 0) { if (selectedCols.has(c)) td.classList.add('selected-col'); }
    else if (selectedRows.size > 0) { if (selectedRows.has(r)) td.classList.add('selected-cell'); }
  });
  const hint = document.getElementById('selection-hint'), info = document.getElementById('selection-info');
  if (!hasSelection()) { hint.classList.remove('show'); return; }
  hint.classList.add('show');
  var parts = [];
  if (selectedCols.size > 0) parts.push('📅 ' + selectedCols.size + '日');
  if (selectedRows.size > 0) parts.push('📦 ' + selectedRows.size + '品目');
...
function updateTooltip(e) {
  ...
  if (selectedCells.size > 0) {
    ...
    dateStrs = Array.from(cellCols).sort(function(a: number, b: number){return a-b;}).map(function(c: number) { return currentDates[c]; }).filter(Boolean);
    prodNames = Array.from(cellRows).sort(function(a: number, b: number){return a-b;}).map(function(r: number) { return currentProducts[r]; }).filter(Boolean);
  ...
  if (left + 220 > window.innerWidth) left = e.clientX - 220;
  if (top + 180 > window.innerHeight) top = e.clientY - 180;
...
function exportToExcel() {
  const filtered = getFilteredData(); if (filtered.length === 0) { alert('データがありません'); return; }
  const dates = getFilteredDates();
  const showTag1 = document.getElementById('show-tag1').checked;
  const showTag2 = document.getElementById('show-tag2').checked;
  const showTag3 = document.getElementById('show-tag3').checked;
  const pivot = {};
  filtered.forEach(function(i) { if (!pivot[i.product]) { pivot[i.product] = { total: 0 }; dates.forEach(function(d) { pivot[i.product][d] = 0; }); } pivot[i.product][i.date] += i.quantity; pivot[i.product].total += i.quantity; });
  const wsData = [], hdr = ['品目名'];
  if (showTag1) hdr.push('#大分類');
  if (showTag2) hdr.push('#中分類');
  if (showTag3) hdr.push('#小分類');
  hdr.push('原価', '売価');
  dates.forEach(function(d) { hdr.push(d); });
  hdr.push('合計');
  wsData.push(hdr);
  Object.keys(pivot).sort().forEach(function(p) {
    const info = (productInfo[p] || {}) as ProductInfo, row = [p];
    if (showTag1) row.push(getTag(p, 1));
    if (showTag2) row.push(getTag(p, 2));
    if (showTag3) row.push(getTag(p, 3));
    row.push(info.cost || '' as any, info.price || '' as any);
...
function exportToCSV() {
  ...
  Object.keys(pivot).sort().forEach(function(p) {
    const info = (productInfo[p] || {}) as ProductInfo;
    csv += '"' + p + '"';
    if (showTag1) csv += ',"' + getTag(p, 1) + '"';
    if (showTag2) csv += ',"' + getTag(p, 2) + '"';
    if (showTag3) csv += ',"' + getTag(p, 3) + '"';
...
function exportTagTemplate() {
  if (currentProducts.length === 0) { showToast('データがありません'); return; }
...
document.onclick = function(e) {
  if (!e.target.closest('.dropdown')) document.querySelectorAll('.dropdown-menu').forEach(function(d) { d.classList.remove('show'); });
